...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('partials/EditItemRelazioniM.html',
    '<ion-view class="" title="Relazione di servizio" left-buttons="leftButtons" right-buttons="rightButtons"><ion-nav-bar class="bar-royal"><ion-nav-buttons side="left"><button class="button button-icon button-clear ion-navicon" ng-click="toggleLeft()"></button></ion-nav-buttons><ion-nav-buttons side="right"><button class="button button-icon button-clear ion-search" ng-click="toggleRight()"></button></ion-nav-buttons></ion-nav-bar><ion-content has-header="true" padding="true"><div class="list"><div class="item  item-balanced"><div ng-show="configAction == \'edit\'" class="panel-heading"><h2><b>Relazione di Servizio : {{item.id}} *</b></h2></div><div ng-show="configAction == \'new\'" class="panel-heading"><h2<b>Creazione nuova Relazione di Servizio</h2<b></div><div ng-show="configAction == \'view\'" class="panel-heading"><h2><b>Relazione di Servizio : {{item.id}} **</b>S</h2></div></div><fancy-select allow-empty="false" label-text="Associazione" value="item.id_utenti" text="item.nome_breve_utenti" header-text="Seleziona utenti" items="utentiList"></fancy-select><fancy-select header-text="Selezione Volontario" label-text="Volontari" allow-empty="false" text="item.elenco_volontari" value="item.elenco_id_volontari" items="volontariList" multi-select="true"></fancy-select><label class="item item-input"><div class="input-label">Data Relazione</div><input type="date" ng-model="item.data_relazioni"></label><label class="item item-input"><div class="input-label">Dalle ore</div><input type="time" ng-model="item.da_ora_relazioni"></label><label class="item item-input"><div class="input-label">Alle ore</div><input type="time" ng-model="item.a_ora_relazioni"></label><label class="item item-input"><span class="input-label">Rapporto</span><input type="text" ng-model="item.rapporto_relazioni"></label><label class="item item-input"><span class="input-label">Auto</span><input type="text" ng-model="item.auto_relazioni"></label><label class="item item-input"><span class="input-label">Note</span><input type="text" ng-model="item.note_relazioni"></label><button class="button button-full button-positive" ng-show="configAction == \'new\'" id="salvabutton" ng-click="save_action(item)" name="singlebutton">Salva Elemento</button><button class="button button-full button-assertive" ng-show="(configAction == \'edit\') && (id_rapporto_valido_servizio == null) && (item.annullato_relazioni == 0)" id="cancelbutton" ng-click="cancel_action(item)" name="button2id">Annulla Elemento</button><button class="button button-full button-positive" ng-show="(item.annullato_relazioni == 1)" id="cancelbutton" name="button2id">Elemento ANNULLATO - nessuna operazione possibile</button><button ng-show="false" class="button button-full button-positive" id="debugBbutton" name="debugButton" ng-click="debug_action(item)">DEBUG</button></div></ion-content></ion-view>');
}]);
})();
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('partials/ListItemM.html',
    '<ion-view class="item-calm" title="Elenco relazioni preventive" left-buttons="leftButtons" right-buttons="rightButtons"><ion-nav-bar class="bar-royal"><ion-nav-buttons side="left"><button class="button button-icon button-clear ion-navicon" ng-click="toggleLeft()"></button></ion-nav-buttons><ion-nav-buttons side="right"><button class="button button-icon ion-more" ng-click="popover.show($event)"></button></ion-nav-buttons></ion-nav-bar><ion-content has-header="true"><div><ion-list><ion-item ng-repeat="item in items" class="item-icon-left"><i ng-show="item.annullato_servizi==1" class="icon ion-close-circled"></i> <i ng-show="item.annullato_servizi==0" class="icon ion-checkmark"></i> <span class="itemTopRight">{{item.data_servizi | date:\'dd/MM/yyyy\' }}</span><p><b>{{item.nome_breve_utenti}}</b></p><span class="itemTopRight">{{item.da_ora_servizi | date:\'HH:mm\' }} {{item.a_ora_servizi | date:\'HH:mm\' }}</span><p>{{item.elenco_volontari}}</p><p>{{item.note_servizi}}</p><p>{{item.rapporto_servizi }}</p><ion-option-button class="button-positive" ng-click="editItem(item.id)">Dettagli</ion-option-button></ion-item></ion-list></div></ion-content></ion-view>');
}]);
})();
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('partials/aboutM.html',
    '<ion-view class="item-dark" title="Informazioni" left-buttons="leftButtons" right-buttons="rightButtons"><ion-nav-bar class="bar-royal"><ion-nav-buttons side="left"><button class="button button-icon button-clear ion-navicon" ng-click="toggleLeft()"></button></ion-nav-buttons></ion-nav-bar><ion-content has-header="true" padding="true" class="bgMain"><div class="list card"><div class="item"><img src="img/cocca2logo.jpg"></div><div class="item"><h1>Provincia di Rimini</h1><h2>Gestione Servizi Volontari</h2></div><a class="item item-icon-left assertive" href="#"><i class="icon ion-code-download"></i> Ruggero Ruggeri</a><div class="item">Utente: {{navTitle}}<br>$location.protocol() = <span ng-bind="{{$location.protocol()}}"></span><br>$location.host() = <span ng-bind="$location.host()"></span><br>$location.port() = <span ng-bind="$location.port()"></span><br>$location.path() = <span ng-bind="$location.path()"></span><br>$location.search() = <span ng-bind="$location.search()"></span><br>$location.hash() = <span ng-bind="$location.hash()"></span><br>base_url = <span>{{base_url}}</span><br></div><div class="item"><button id="btn-testconnection" class="button button-block button-positive" ng-click="test_connection()">Test connection</button></div></div></ion-content></ion-view>');
}]);
})();
